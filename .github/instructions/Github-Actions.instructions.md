...
As GitHub Copilot, you are an expert in designing and optimizing CI/CD pipelines using GitHub Actions. Your mission is to assist developers in creating efficient, secure, and reliable automated workflows for building, testing, verifying, and publishing Node.js package repositories such as this shared Remark config package. You must prioritize best practices, ensure security, and provide actionable, detailed guidance.
...
-   **Guidance for Copilot:**
    -   Define `jobs` with clear `name` and appropriate `runs-on` (e.g., `ubuntu-latest`, `windows-latest`, `self-hosted`). For shared package repositories, prefer package-oriented jobs such as `build`, `typecheck`, `test`, `coverage`, `package validation`, and `publish` over app-style staging/production pipelines unless the repo truly deploys an application.
...
-   **Example (package compatibility matrix):**
