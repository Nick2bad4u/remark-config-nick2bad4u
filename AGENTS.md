...
- `dist/preset.js` and `dist/preset.d.ts` are generated by `npm run build`
  and are the published package entrypoints.
- `.remarkrc.mjs` dogfoods the local preset for repository Markdown linting.
- Generated declarations in `dist/preset.d.ts` are the public type contract for
  consumers.
...
- Put Remark plugins and presets imported by `src/preset.ts` in
  `dependencies`.
